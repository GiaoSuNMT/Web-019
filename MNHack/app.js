...
mongoose.connect("mongodb://localhost:27017/Point", async err => {
  ...
  server.get("/get-score/:id", async (req, res) => {
    const id = req.params.id;
    const roundplayer = await point.find({ gameID: id });
    res
      .status(200)
      .json({
        point1: roundplayer[0].point,
        point2: roundplayer[1].point,
        point3: roundplayer[2].point,
        point4: roundplayer[3].point
      });
  });
